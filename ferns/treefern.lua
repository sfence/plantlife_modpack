...
abstract_ferns.grow_tree_fern = function(pos)
	...
	if name ~= "air" and name ~= "hades_ferns:sapling_tree_fern"
	...
	size = size + 1
	local crown = ({ "hades_ferns:tree_fern_leaves", "hades_ferns:tree_fern_leaves_02" })[math.random(1, 2)]
	...
	while (i < size) do
		...
		if not (name == "air" or (i == 1 and name == "hades_ferns:sapling_tree_fern")) then
			brk = true
			break
		end
		minetest.swap_node({x = pos.x, y = pos.y + i, z = pos.z}, { name = "hades_ferns:fern_trunk" })
		i = i + 1
	end
	if not brk then
...
minetest.register_node("hades_ferns:tree_fern_leaves", {
		...
		items = {
			{
				...
				--  reharvesting leaves)
				items = {"hades_ferns:sapling_tree_fern"},
				rarity = 10,
			},
			{
				items = {"hades_ferns:sapling_tree_fern"},
			...
			{
				items = {"hades_ferns:tree_fern_leaves"},
			}
		}
	},
...
minetest.register_node("hades_ferns:tree_fern_leaves_02", {
	drawtype = "plantlike",
	visual_scale = math.sqrt(8),
	paramtype = "light",
...
-- FERN TRUNK
-----------------------------------------------------------------------------------------------
minetest.register_node("hades_ferns:fern_trunk", {
	...
	after_destruct = function(pos,oldnode)
        ...
        if node.name == "hades_ferns:fern_trunk" then
            minetest.dig_node({x=pos.x,y=pos.y+1,z=pos.z})
            minetest.add_item(pos,"hades_ferns:fern_trunk")
        end
    end,
})
...
-- TREE FERN SAPLING
-----------------------------------------------------------------------------------------------
minetest.register_node("hades_ferns:sapling_tree_fern", {
...
minetest.register_abm({
	nodenames = "hades_ferns:sapling_tree_fern",
	interval = 1000,
	chance = 4,
